...
export function registerCleanupTool(server: McpServer, db: Database.Database): void {
  server.tool(
    ...
    wrapToolHandler('kb_cleanup', ({ prune, max_fact_age_days }) => {
      ...
      return formatSingleResponse(
        ...
        `Found ${deletedRepos.length} deleted repos (${pruneStatus}), ${staleFacts.length} stale facts flagged for review`,
      );
    }),
  );
}
